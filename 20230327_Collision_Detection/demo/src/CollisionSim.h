...
class CollisionSim : public Simulation {
   public:
    ...
    virtual bool advance() override {
        ...
        //// to save the origin data
        //std::vector<Eigen::Vector3d> LinearMomentum;
        //std::vector<Eigen::Vector3d> AngularMomentum;
        //std::vector<Eigen::Vector3d> Force;
        //std::vector<Eigen::Vector3d> Torque;
        //std::vector<Eigen::Vector3d> AngularVelocity;
        //std::vector<Eigen::Vector3d> LinearVelocity;
        ...
        //    LinearMomentum.push_back(m_objects[i].getLinearMomentum());
        //    AngularMomentum.push_back(m_objects[i].getAngularMomentum());
        //    Force.push_back(m_objects[i].getForce());
        //    Torque.push_back(m_objects[i].getTorque());
        //    AngularVelocity.push_back(m_objects[i].getAngularVelocity());
        //    LinearVelocity.push_back(m_objects[i].getLinearVelocity());
        //    Position.push_back(m_objects[i].getPosition());
        //    Rotation.push_back(m_objects[i].getRotation());

        //    // integrate velocities
        //    m_objects[i].setLinearMomentum(LinearMomentum[i] + m_dt * Force[i]);
        //    m_objects[i].setAngularMomentum(AngularMomentum[i] + m_dt * Torque[i]);
        //    m_objects[i].resetForce();
        //    m_objects[i].resetTorque();

        //    // angular velocity (matrix)
        //    Eigen::Vector3d w = AngularVelocity[i];
        //    Eigen::Quaterniond wq;
        //    wq.w() = 0;
        //    wq.vec() = w;

        //    // integrate position and rotation
        //    m_objects[i].setPosition(Position[i] + m_dt * LinearVelocity[i]);
        //    Eigen::Quaterniond q = Rotation[i];
        //    Eigen::Quaterniond dq = wq * q;
        //    Eigen::Quaterniond new_q;
        //    new_q.w() = q.w() + 0.5 * m_dt * dq.w();
        //    new_q.vec() = q.vec() + 0.5 * m_dt * dq.vec();
        //    m_objects[i].setRotation(new_q.normalized());
        //}

        //// compute the collision detection
        //m_collisionDetection.computeCollisionDetection(m_broadPhaseMethod, m_narrowPhaseMethod, m_eps);
        //int count = m_collisionDetection.m_contacts.size();
        //double substep;
        //if (count > 0)
        //{
        ...
        //    double left = 0;
        //    double right = m_dt;
        //    double torlerance = m_dt / 20;
        //    while (right - left > torlerance)
        //    {
        ...
        //            // integrate velocities
        //            m_objects[i].setLinearMomentum(LinearMomentum[i] + mid * Force[i]);
        //            m_objects[i].setAngularMomentum(AngularMomentum[i] + mid * Torque[i]);
        //            m_objects[i].resetForce();
        //            m_objects[i].resetTorque();
        ...
        //            // integrate position and rotation
        //            m_objects[i].setPosition(Position[i] + mid * LinearVelocity[i]);
        //            Eigen::Quaterniond q = Rotation[i];
        //            Eigen::Quaterniond dq = wq * q;
        //            Eigen::Quaterniond new_q;
        //            new_q.w() = q.w() + 0.5 * mid * dq.w();
        //            new_q.vec() = q.vec() + 0.5 * mid * dq.vec();
        //            m_objects[i].setRotation(new_q.normalized());
        //        }
        //        m_collisionDetection.computeCollisionDetection(m_broadPhaseMethod, m_narrowPhaseMethod, m_eps);
        //        if (m_collisionDetection.m_contacts.size() < count)
        //        {
        //            left = mid;
        //        }
        //        else 
        ...
        //for (int i = 0; i < m_objects.size(); i++) {
        //    // integrate velocities
        //    m_objects[i].setLinearMomentum(LinearMomentum[i] + substep * Force[i]);
        //    m_objects[i].setAngularMomentum(AngularMomentum[i] + substep * Torque[i]);
        //    m_objects[i].resetForce();
        //    m_objects[i].resetTorque();
        ...
        //    // integrate position and rotation
        //    m_objects[i].setPosition(Position[i] + substep * LinearVelocity[i]);
        //    Eigen::Quaterniond q = Rotation[i];
        //    Eigen::Quaterniond dq = wq * q;
        //    Eigen::Quaterniond new_q;
        //    new_q.w() = q.w() + 0.5 * substep * dq.w();
        //    new_q.vec() = q.vec() + 0.5 * substep * dq.vec();
        //    m_objects[i].setRotation(new_q.normalized());
        //}

        //m_collisionDetection.computeCollisionDetection(m_broadPhaseMethod, m_narrowPhaseMethod, m_eps);

        //m_collisionDetection.applyImpulse(m_eps);

        //substep = m_dt - substep;
